...
import com.brackeen.javagamebook.tilegame.sprites.Monster;
...
public class GameManager extends GameCore {
	...
	public static void main(String[] args) {
		...
		// run is calling to init and gameloop in the order
	...
	public void init() {
		super.init(); // call to implement code in Parent class
	...
	public void update(long elapsedTime) {
		Monster player = (Monster) map.getPlayer();

		// player is dead! start map over
		if (player.getState() == Monster.STATE_DEAD) {
		...
		while (i.hasNext()) {
			...
			if (sprite instanceof Monster) {
				Monster creature = (Monster) sprite;
				if (creature.getState() == Monster.STATE_DEAD) {
					i.remove();
				} else {
	 ...
	 */
	private void updateCreature(Monster creature, long elapsedTime) {
	...
	public boolean isCollision(Sprite s1, Sprite s2) {
		...
		if (s1 instanceof Monster && !((Monster) s1).isAlive()) {
			return false;
		}
		if (s2 instanceof Monster && !((Monster) s2).isAlive()) {
			return false;
		}
	...
	public void checkPlayerCollision(Player player, boolean canKill) {
		...
		} else if (collisionSprite instanceof Monster) {
			Monster badguy = (Monster) collisionSprite;
			if (canKill) {
				// kill the badguy and make player bounce
				soundManager.play(boopSound);
				badguy.setState(Monster.STATE_DYING);
				player.setY(badguy.getY() - player.getHeight());
				player.jump(true);
			} else {
				// player dies!
				player.setState(Monster.STATE_DYING);
			}
		}
	}
